...
export default function FrequenciesManagement() {
  ...
  const [editingFrequency, setEditingFrequency] = useState<FrequencyForm | null>(null);
  ...
  const handleEdit = (frequency: any) => {
    ...
    setFormData({
      ...
      usageGuidelines: frequency.usageGuidelines || '',
      ...
      <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
        {frequencies.map((freq) => (
          <View key={freq.id} style={styles.frequencyCard}>
            <View style={[styles.colorIndicator, { backgroundColor: freq.color }]} />
            ...
            <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.formGroup}>
                ...
                <View style={styles.categoryOptions}>
                  {['solfeggio', 'chakra', 'brainwave', 'healing', 'sleep', 'manifestation'].map((cat) => (
                    <TouchableOpacity
                      key={cat}
                      style={[
              ...
              <View style={styles.formGroup}>
                ...
                <Text style={[styles.label, { fontSize: 12, marginTop: 8, marginBottom: 4 }]}>Best Time</Text>
                <TextInput
                  style={styles.input}
                  value={formData.usageInstructions?.bestTime || ''}
                  onChangeText={(text) => setFormData({ 
                    ...formData, 
                    usageInstructions: { ...formData.usageInstructions, bestTime: text }
                  })}
                  placeholder="e.g., Morning for grounding, evening for relaxation"
                  placeholderTextColor="#6B7280"
                />
                
                <Text style={[styles.label, { fontSize: 12, marginTop: 8, marginBottom: 4 }]}>Environment</Text>
                <TextInput
                  style={styles.input}
                  value={formData.usageInstructions?.environment || ''}
                  onChangeText={(text) => setFormData({ 
                    ...formData, 
                    usageInstructions: { ...formData.usageInstructions, environment: text }
                  })}
                  placeholder="e.g., Quiet space with dim lighting"
                  placeholderTextColor="#6B7280"
                />
                
                <Text style={[styles.label, { fontSize: 12, marginTop: 8, marginBottom: 4 }]}>Preparation</Text>
                <TextInput
                  style={styles.input}
                  value={formData.usageInstructions?.preparation || ''}
                  onChangeText={(text) => setFormData({ 
                    ...formData, 
                    usageInstructions: { ...formData.usageInstructions, preparation: text }
                  })}
                  placeholder="e.g., Deep breathing for 2 minutes, set intention"
                  placeholderTextColor="#6B7280"
                />
              </View>
